...
const int sz = 13;	// szerokość planszy
const int wy = 13;	// wysokość planszy

const int k_sz = 35;	// szerokość (piksele) kafelka
const int k_wy = 35;	// wysokość (piksele) kafelka
...
// Każdy z graczy dostaje jeden punkt energii co 1/FPS sekundy

const int energia_ruchu = 7;	// ruch można wykonać tylko gdy energia >= 7
const int dlugosc_lontu = 70;	// czas do wybuchu bomby
const int czas_wybuchu = 14;	// długość eksplozji
        ...
        for (int j = 0; j < wy; j++) {
            ...
            for (int k = 0; k < 2; k++) {
                if (p.g[k].x == i && p.g[k].y == j) {
                    ...
                    al_draw_bitmap_region(bitmapa[gracz], (p.g[k].kolor * 4) * k_sz,
                                          0, k_sz, k_wy, i*k_sz, j*k_sz, 0);
                }
            }
